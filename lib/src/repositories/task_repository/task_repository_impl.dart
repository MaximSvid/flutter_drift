...
class TaskRepositoryImplementation implements TaskRepository {
  ...
  Stream<List<Task>> getTasks() {
    // Здесь все без изменений, просто возвращаем поток задач из локального источника
  ...
  Future<void> createTask(TasksCompanion task) async {
    // Просто создаем задачу локально. Статус `pendingCreate` выставится по умолчанию.
    await localDataSource.createTask(task);
    // Запускаем синхронизацию "в фоне". Не ждем ее завершения.
    syncService.sync();
  }

  // @override
  // Future<void> updateTask(Task task) async {
  //   // Обновляем задачу локально. Статус `pendingUpdate` выставится по умолчанию.
  ...
  //   // 
  //   await localDataSource.updateTask(task.copyWith(syncStatus: ))
  // }
  ...
  //     } else {
  //       print(
  //         'Failed to send task to server: ${response.statusCode} - ${response.body}',
  //       );
  //     }
  //   } catch (e) {
  ...
  //   final url = Uri.parse(
  //     'http://10.0.2.2:8080/api/tasks/${entry.id}',
  //   ); // Use 10.0.2.2 for Android emulator
  //   try {
  ...
  //     } else {
  //       print(
  //         'Failed to send task update to server: ${response.statusCode} - ${response.body}',
  //       );
  //     }
  //   } catch (e) {
  ...
  //     } else {
  //       print(
  //         'Failed to send task delete to server: ${response.statusCode} - ${response.body}',
  //       );
  //     }
  //   } catch (e) {
